...
@property (strong, nonatomic) Live2DSprite *sprite;
...
- (void)viewDidLoad {
    ...
    _sprite = [[Live2DSprite alloc] initWithFrame:CGRectMake(ScreenWidth - 320, ScreenHeight - 720, 480, 720)];
    [_sprite loadModelName:@"Shanbao"];
...
- (void)captureOutput:(AVCaptureOutput *)output didOutputSampleBuffer:(nonnull CMSampleBufferRef)sampleBuffer fromConnection:(nonnull AVCaptureConnection *)connection {
    ...
    if (detectResult != nil && detectResult.count > 0) {
        [self.sprite setFaceDetectionData:[detectResult firstObject]];
    }
